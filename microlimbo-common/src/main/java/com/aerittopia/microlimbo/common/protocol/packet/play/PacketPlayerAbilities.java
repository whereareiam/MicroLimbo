...
public class PacketPlayerAbilities implements PacketOut {
	private final int flags = 0x02;
	private final float flyingSpeed = 0.0F;
	private final float fieldOfView = 0.1F;

	@Override
	public void encode(ByteMessage message, Version version) {
		message.writeByte(flags);
		message.writeFloat(flyingSpeed);
		message.writeFloat(fieldOfView);
	}
}
