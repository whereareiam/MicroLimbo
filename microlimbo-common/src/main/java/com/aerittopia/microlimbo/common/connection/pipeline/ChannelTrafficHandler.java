...
public class ChannelTrafficHandler extends ChannelInboundHandlerAdapter {
	...
	public ChannelTrafficHandler(LoggingHelper loggingHelper, int maxPacketSize, double interval, double maxPacketRate) {
	...
	@Override
	public void channelRead(@NotNull ChannelHandlerContext ctx, @NotNull Object object) throws Exception {
		if (object instanceof ByteBuf in) {
			...
			if (packetBucket != null) {
				...
				if (packetBucket.getCurrentPacketRate() > maxPacketRate) {
					closeConnection(ctx, "Closed %s due to many packet sent (%d in the last %.1f seconds)", ctx.channel().remoteAddress(), packetBucket.sum, (packetBucket.intervalTime / 1000.0));
					return;
				}
			}
		}
	...
	private void closeConnection(ChannelHandlerContext ctx, String reason, Object... args) {
		ctx.close();
		loggingHelper.info(reason, args);
	}
